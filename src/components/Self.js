...
import { useAuth, BaseURL } from "../middleware/auth";
function Self() {
  const { accountData, userData, callUserAuthentication, callGetAccountData } =
    useAuth();
  useEffect(() => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const debitDetail = await fetch(
        BaseURL + "/api/account/updateAccountBalance",
        {
          method: "POST",
          headers: {
      ...
      const response = await fetch(BaseURL + "/api/account/addTransaction", {
        method: "POST",
        headers: {
          "Content-Type": "Application/json",
        },
